...
async function grab(label, ws, layer, out) {
  console.log('› fetching', label, '…');
  const res = await fetch(wmsUrl(ws, layer));
...
await grab('official contour lines (EL.ContourLine)', 'el', 'EL.ContourLine', 'ov-contours-walferdange.png');
...
console.log('✓ overlays baked (' + WIDTH + '×' + HEIGHT + ')');
